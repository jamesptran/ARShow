...
class SolarCalculator {
    ...
    func dayfrac_to_local_hr(dayfrac: Double, timezone : Double) -> Double{
        return 24.0*limit_zero2one(value: dayfrac + timezone/24.0)
    }
    ...
    func julian_day( year : Int,  month : Int,  day : Int,  hour : Int,  minute : Int,  second : Double,  dut1 : Double,  tz : Double) -> Double {
    ...
    func earth_heliocentric_longitude(jme : Double) ->Double
    {
        ...
        {
            sum.append(earth_periodic_term_summation(terms: L_TERMS[i], jme: jme))
        }
        return limit_degrees(degrees: rad2deg(radians: earth_values(term_sum: sum, jme: jme)))
    }
    ...
    func earth_heliocentric_latitude(jme : Double) ->Double
    {
        ...
        {
            sum.append(earth_periodic_term_summation(terms: B_TERMS[i], jme: jme))
        }
        return limit_degrees(degrees: rad2deg(radians: earth_values(term_sum: sum, jme: jme)))
    }
    ...
    func earth_radius_vector(jme : Double) -> Double
    {
        ...
        {
            sum.append(earth_periodic_term_summation(terms: R_TERMS[i], jme: jme))
        }
    ...
    {
        return third_order_polynomial(a: 1.0/189474.0, b: -0.0019142, c: 445267.11148, d: 297.85036, x: jce)
    }
    
    func mean_anomaly_sun(jce : Double) -> Double
    {
        return third_order_polynomial(a: -1.0/300000.0, b: -0.0001603, c: 35999.05034, d: 357.52772, x: jce)
    }
    
    func mean_anomaly_moon(jce : Double) -> Double
    {
        return third_order_polynomial(a: 1.0/56250.0, b: 0.0086972, c: 477198.867398, d: 134.96298, x: jce)
    }
    
    func argument_latitude_moon(jce : Double) -> Double
    {
        return third_order_polynomial(a: 1.0/327270.0, b: -0.0036825, c: 483202.017538, d: 93.27191, x: jce)
    }
    
    func ascending_longitude_moon(jce : Double) -> Double
    {
        return third_order_polynomial(a: 1.0/450000.0, b: 0.0020708, c: -1934.136261, d: 125.04452, x: jce)
    }
    ...
    func nutation_longitude_and_obliquity(jce : Double, x : [Double]) -> (del_psi : Double, del_epsilon : Double)
    {
        ...
        {
            xy_term_sum = deg2rad(degrees: xy_term_summation(i: i, x: x))
    ...
    {
        return limit_degrees(degrees: 280.46061837 + 360.98564736629 * (jd - 2451545.0) + jc*jc*(0.000387933 - jc/38710000.0))
    }
    
    func greenwich_sidereal_time (nu0 : Double, delta_psi : Double, epsilon : Double) -> Double
    {
        return nu0 + delta_psi*cos(deg2rad(degrees: epsilon))
    }
    
    func geocentric_right_ascension(lamda : Double, epsilon : Double, beta : Double) -> Double
    {
        var lamda_rad : Double = deg2rad(degrees: lamda)
        var epsilon_rad : Double = deg2rad(degrees: epsilon)
        return limit_degrees(degrees: rad2deg(radians: atan2(sin(lamda_rad)*cos(epsilon_rad) - tan(deg2rad(degrees: beta))*sin(epsilon_rad), cos(lamda_rad))))
    }
    
    
    func geocentric_declination(beta : Double, epsilon : Double, lamda : Double) -> Double
    {
        var beta_rad : Double = deg2rad(degrees: beta)
        var epsilon_rad : Double = deg2rad(degrees: epsilon)
        return rad2deg(radians: asin(sin(beta_rad)*cos(epsilon_rad) + cos(beta_rad)*sin(epsilon_rad)*sin(deg2rad(degrees: lamda))))
    }
    
    
    func observer_hour_angle(nu : Double, longitude : Double, alpha_deg : Double) -> Double
    {
        return limit_degrees(degrees: nu + longitude - alpha_deg)
    }
    ...
    {
        var lat_rad : Double = deg2rad(degrees: latitude)
        var xi_rad : Double = deg2rad(degrees: xi)
        var h_rad : Double = deg2rad(degrees: h)
        var delta_rad : Double = deg2rad(degrees: delta)
        ...
        var delta_prime : Double = rad2deg(radians: atan2(Double((sin(delta_rad) - y * sin(xi_rad)) * cos(delta_alpha_rad)), Double(cos(delta_rad) - x * sin(xi_rad) * cos(h_rad))))
        var delta_alpha : Double = rad2deg(radians: delta_alpha_rad)
    ...
    {
        var lat_rad : Double = deg2rad(degrees: latitude)
        var delta_prime_rad : Double = deg2rad(degrees: delta_prime)
        return rad2deg(radians: asin(sin(lat_rad)*sin(delta_prime_rad) + cos(lat_rad)*cos(delta_prime_rad) * cos(deg2rad(degrees: h_prime))))
    }
    ...
    func atmospheric_refraction_correction(pressure : Double, temperature : Double, atmos_refract : Double, e0 : Double)  -> Double
    {
        ...
        if (e0 >= -1*(sun_radius + atmos_refract)){
            del_e = (pressure / 1010.0) * (283.0 / (273.0 + temperature)) * 1.02 / (60.0 * tan(deg2rad(degrees: e0 + 10.3/(e0 + 5.11))))
        }
        return del_e
    }
    ...
    {
        var h_prime_rad  : Double = deg2rad(degrees: h_prime)
        var lat_rad  : Double = deg2rad(degrees: latitude)
        return limit_degrees(degrees: rad2deg(radians: atan2(sin(h_prime_rad), cos(h_prime_rad)*sin(lat_rad) - tan(deg2rad(degrees: delta_prime))*cos(lat_rad))))
    }
    
    func topocentric_azimuth_angle(azimuth_astro : Double) -> Double
    {
        return limit_degrees(degrees: azimuth_astro + 180.0)
    }
    
    
    func surface_incidence_angle(zenith : Double, azimuth_astro : Double, azm_rotation : Double, slope : Double) -> Double
    {
        var zenith_rad : Double = deg2rad(degrees: zenith)
        var slope_rad : Double = deg2rad(degrees: slope)
        return rad2deg(radians: acos(cos(zenith_rad)*cos(slope_rad) + sin(slope_rad )*sin(zenith_rad) * cos(deg2rad(degrees: azimuth_astro - azm_rotation))))
    }
    
    func sun_mean_longitude(jme : Double) -> Double
    {
        return limit_degrees(degrees: 280.4664567 + jme*(360007.6982779 + jme*(0.03032028 + jme*(1/49931.0 + jme*(-1/15300.0 + jme*(-1/2000000.0))))))
    }
    
    func eot(m : Double, alpha : Double, del_psi : Double, epsilon : Double) -> Double
    {
        return limit_minutes(minutes: 4.0*(m - 0.0057183 - alpha + del_psi*cos(deg2rad(degrees: epsilon))))
    }
    ...
    {
        var h0 : Double = -99999
        var latitude_rad : Double = deg2rad(degrees: latitude)
        var delta_zero_rad : Double = deg2rad(degrees: delta_zero)
        var argument : Double = (sin(deg2rad(degrees: h0_prime)) - sin(latitude_rad)*sin(delta_zero_rad)) / (cos(latitude_rad)*cos(delta_zero_rad))
        
        if (abs(argument) <= 1)
        {
            h0 = limit_degrees180(degrees: rad2deg(radians: acos(argument)))
        }
    ...
    func approx_sun_rise_and_set(sun_transit : Double , h0 : Double) -> (sunrise : Double, sunset : Double, transit : Double)
    {
        ...
        var sunrise : Double = limit_zero2one(value: sun_transit - h0_dfrac);
        var sunset : Double = limit_zero2one(value: sun_transit + h0_dfrac);
        var transit : Double = limit_zero2one(value: sun_transit)
    ...
    func rts_alpha_delta_prime(ad : [Double], n : Double) -> Double
    {
        ...
        if abs(a) >= 2.0 { a = limit_zero2one(value: a) }
        if abs(b) >= 2.0 { b = limit_zero2one(value: b) }
    ...
    func rts_sun_altitude(latitude : Double, delta_prime : Double, h_prime : Double) -> Double
    {
        var latitude_rad : Double = deg2rad(degrees: latitude)
        var delta_prime_rad : Double = deg2rad(degrees: delta_prime)
        return rad2deg(radians: asin(sin(latitude_rad)*sin(delta_prime_rad) + cos(latitude_rad)*cos(delta_prime_rad)*cos(deg2rad(degrees: h_prime))))
    }
    ...
    {
        return m_rts[sun] + (h_rts[sun] - h0_prime) / (360.0*cos(deg2rad(degrees: delta_prime[sun]))*cos(deg2rad(degrees: latitude))*sin(deg2rad(degrees: h_prime[sun])))
    }
    ...
    {
        var x : [Double] = [Double](repeating: 0.0, count: TERM_X_COUNT)
        jc = julian_century(jd: jd)
        jde = julian_ephemeris_day(jd: jd, delta_t: delta_t)
        jce = julian_ephemeris_century(jde: jde)
        jme = julian_ephemeris_millennium(jce: jce)
        l = earth_heliocentric_longitude(jme: jme)
        b = earth_heliocentric_latitude(jme: jme)
        r = earth_radius_vector(jme: jme)
        theta = geocentric_longitude(l: l)
        beta = geocentric_latitude(b: b)
        
        x0 = mean_elongation_moon_sun(jce: jce)
        x[TERM_X0] = x0
        
        x1 = mean_anomaly_sun(jce: jce)
        x[TERM_X1] = x1
        
        x2 = mean_anomaly_moon(jce: jce)
        x[TERM_X2] = x2
        
        x3 = argument_latitude_moon(jce: jce)
        x[TERM_X3] = x3
        
        x4 = ascending_longitude_moon(jce: jce)
        x[TERM_X4] = x4
        
        var result : (del_psi : Double, del_epsilon : Double) = nutation_longitude_and_obliquity(jce: jce, x: x)
        del_psi = result.del_psi
        del_epsilon = result.del_epsilon
        
        epsilon0 = ecliptic_mean_obliquity(jme: jme)
        epsilon = ecliptic_true_obliquity(delta_epsilon: del_epsilon, epsilon0: epsilon0)
        del_tau = aberration_correction(r: r)
        lamda = apparent_sun_longitude(theta: theta, delta_psi: del_psi, delta_tau: del_tau)
        nu0 = greenwich_mean_sidereal_time (jd: jd, jc: jc)
        nu = greenwich_sidereal_time (nu0: nu0, delta_psi: del_psi, epsilon: epsilon)
        alpha = geocentric_right_ascension(lamda: lamda, epsilon: epsilon, beta: beta)
        delta = geocentric_declination(beta: beta, epsilon: epsilon, lamda: lamda)
    }
    ...
    func calculate_eot_and_sun_rise_transit_set()
    {
        ...
        var alpha : [Double] = [Double](repeating : 0.0, count : JD_COUNT)
        var delta : [Double] = [Double](repeating : 0.0, count : JD_COUNT)
        
        var m_rts : [Double] = [Double](repeating : 0.0, count : SUN_COUNT)
        var nu_rts : [Double] = [Double](repeating : 0.0, count : SUN_COUNT)
        var h_rts : [Double] = [Double](repeating : 0.0, count : SUN_COUNT)
        var alpha_prime : [Double] = [Double](repeating : 0.0, count : SUN_COUNT)
        var delta_prime : [Double] = [Double](repeating : 0.0, count : SUN_COUNT)
        var h_prime : [Double] = [Double](repeating : 0.0, count : SUN_COUNT)
        var h0_prime : Double = -1*(sun_radius + atmos_refract)
        
        var i : Int;
        
        m = sun_mean_longitude(jme: jme);
        eot = eot(m: m, alpha: self.alpha, del_psi: del_psi, epsilon: epsilon)
        ...
        var jd_prev = jd
        jd = julian_day (year: year, month: month, day: day, hour: 0, minute: 0, second: 0, dut1: 0, tz: 0) // Calculate the julian day at midnight in Grenwich
        ...
        m_rts[SUN_TRANSIT] = approx_sun_transit_time(alpha_zero: alpha[JD_ZERO], longitude: longitude, nu: nu_local)
        h0 = sun_hour_angle_at_rise_set(latitude: latitude, delta_zero: delta[JD_ZERO], h0_prime: h0_prime);
        if (h0 >= 0) {
            var result : (sunrise : Double, sunset : Double, transit : Double) = approx_sun_rise_and_set(sun_transit: m_rts[SUN_TRANSIT], h0: h0)
            
            m_rts[SUN_RISE] = result.sunrise
            m_rts[SUN_TRANSIT] = result.transit
            m_rts[SUN_SET] = result.sunset
            
            for i in 0...(SUN_COUNT - 1) {
                ...
                h_prime[i] = limit_degrees180pm(degrees: nu_rts[i] + longitude - alpha_prime[i]);
                h_rts[i] = rts_sun_altitude(latitude: latitude, delta_prime: delta_prime[i], h_prime: h_prime[i]);
            }
            srha = h_prime[SUN_RISE];
            ssha = h_prime[SUN_SET];
            sta = h_rts[SUN_TRANSIT];
            suntransit = dayfrac_to_local_hr(dayfrac: m_rts[SUN_TRANSIT] - h_prime[SUN_TRANSIT] / 360.0, timezone: timezone);
            sunrise = dayfrac_to_local_hr(dayfrac: sun_rise_and_set(m_rts: m_rts, h_rts: h_rts, delta_prime: delta_prime, latitude: latitude, h_prime: h_prime, h0_prime: h0_prime, sun: SUN_RISE), timezone: timezone);
            sunset = dayfrac_to_local_hr(dayfrac: sun_rise_and_set(m_rts: m_rts, h_rts: h_rts, delta_prime: delta_prime, latitude: latitude, h_prime: h_prime, h0_prime: h0_prime, sun: SUN_SET), timezone: timezone);
        }
        else
        {
            srha = -99999
    ...
    func spa_calculate() -> Int
    {
        ...
        if (result == 0)
        {
            jd = julian_day (year: year, month: month, day: day, hour: hour, minute: minute, second: second, dut1: delta_ut1, tz: timezone)
            calculate_geocentric_sun_right_ascension_and_declination()
            h = observer_hour_angle(nu: nu, longitude: longitude, alpha_deg: alpha)
            xi = sun_equatorial_horizontal_parallax(r: r)
            var rapt : (delta_prime : Double, delta_alpha : Double) = right_ascension_parallax_and_topocentric_dec(latitude: latitude, elevation: elevation, xi: xi, h: h, delta: delta)
            del_alpha = rapt.delta_alpha
            delta_prime = rapt.delta_prime
            
            alpha_prime = topocentric_right_ascension(alpha_deg: alpha, delta_alpha: del_alpha)
            
            h_prime = topocentric_local_hour_angle(h: h, delta_alpha: del_alpha)
            e0 = topocentric_elevation_angle(latitude: latitude, delta_prime: delta_prime, h_prime: h_prime)
            del_e = atmospheric_refraction_correction(pressure: pressure, temperature: temperature, atmos_refract: atmos_refract, e0: e0)
            e = topocentric_elevation_angle_corrected(e0: e0, delta_e: del_e)
            zenith = topocentric_zenith_angle(e: e)
            azimuth_astro = topocentric_azimuth_angle_astro(h_prime: h_prime, latitude: latitude, delta_prime: delta_prime)
            azimuth = topocentric_azimuth_angle(azimuth_astro: azimuth_astro)
            incidence = surface_incidence_angle(zenith: zenith, azimuth_astro: azimuth_astro, azm_rotation: azm_rotation, slope: slope)
